...
// Application Starts
function generateWoodyLipsum(nop, len) {
    var movieArr = ['anniehall', 'bluejasmine', 'manhattanmurder', 'midnightinparis', 'youwill'],
        randomNum = Math.floor(Math.random() * movieArr.length),
        randomMovie = movieArr[randomNum],
        tempArrLength = 0,
        finalText = '';
    //console.log(randomNum, randomMovie);
    $.ajax({
        url: 'woody.json',
        dataType: 'json',
        success: function (data) {
            ...
            for (i; i < dataLength; i += 1) {
                if (data[i]['length'] === len && data[i]['movie'] === randomMovie) {
                    tempArr.push(data[i]);
                }
            }
            imgPath = '/assets/img/movie/' + tempArr[0]['movie'] + '.jpg';
            tempArrLength = tempArr.length;
            for (j; j < nop; j += 1) {
                if (j !== 0) {
                    finalText += '\n\n';
                }
                if (j >= tempArrLength - 1) {
                    finalText += tempArr[idx]['text'];
                    idx = 0;
                } else {
                    finalText += tempArr[idx]['text'];
                    idx += 1;
            ...
            $('.lipsum-area').val(finalText);
            $('.image-holder').html('').append('<img src="' + imgPath + '" style="display:none;">').find('img').fadeIn();
        }
    });
}
...
$(function () {
    $('.lipsum-area').val('');
    $('[name="length"]:first').click();
});

/* Generate */
$('.generate').on('click', function () {
    var nop = parseInt($('.paragraph-count').val(), 10),
        len = parseInt($('[name="length"]:checked').val(), 10);
    generateWoodyLipsum(nop, len);
});

$('.try-another').on('click', function () {
    if ($('.lipsum-area').val() === '') {
        alert('"All I have in life is my imagination"\n\nWoody Allen');
    } else {
        $('.generate').click();
    }
});
...
/* Copy to clipboard */
$('.copy').clipboard({
    path: '/assets/js/jquery.clipboard.swf',
    copy: function () {
        return $('.lipsum-area').val();
    }
});
